...
window.TrainingPage = {
    ...
    _kpiHTML() {
        ...
        const totalTrains = Object.values(apiCounts).reduce((s, n) => s + (Number(n) || 0), 0);
        ...
        return `
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                ${UI.kpiCard('fas fa-dumbbell', '本周训练总次数', totalTrains, '公司新闻 API', 'accent')}
    ...
    _employeeTableHTML() {
        const headers = [
            ...
            { key: 'trainsCount', label: '本周训练次数 (API)', sortable: true },
            { key: 'type', label: '训练类型' },
        ...
        const rows = this.employees.map(emp => {
            ...
            const totalTrains = apiCounts[String(emp.player_id)] || 0;
